...
async def change_role(bot, discord_id, new_role_id, current_role_id=0):
    ...
    if current_role_id != 0:
        await member.remove_roles(discord.utils.get(bot.lvguild.roles, id=current_role_id))
    await member.add_roles(discord.utils.get(bot.lvguild.roles, id=new_role_id))
    
async def send_rolechange_msg(bot, notikums, discord_id, role=0, osu_id=None, osu_user=None):
