    ...
    async def refresh_roles(self):
        try:
            ...
            async with self.bot.db.pool.acquire() as db:
                ...
                result = await db.fetch('SELECT discord_id, osu_id FROM players WHERE osu_id IS NOT NULL;')
                ...
                for row in result:
                    ...
                    if country_rank == 99999:
                        ...
                        if osu_user == {'error': None}:
                            ...
                            if osu_api_check == {'error': None}:
                                continue
                            if current_role == []:
                                await change_role(bot = self.bot, discord_id = row[0], new_role_id=ROLES['restricted'])
                                await send_rolechange_msg(bot = self.bot, discord_id=row[0], notikums='restricted', osu_user=osu_user)
                                continue
                            if ROLES[current_role[0]] != ROLES['restricted']:
                                await change_role(bot = self.bot, discord_id = row[0], current_role_id=ROLES[current_role[0]], new_role_id=ROLES['restricted'])
                                await send_rolechange_msg(bot = self.bot, discord_id=row[0], notikums='restricted', osu_user=osu_user)
                            continue

                        if not osu_user['statistics']['is_ranked']:
                            if current_role == []:
                                await change_role(bot = self.bot, discord_id = row[0], new_role_id=ROLES['inactive'])
                                await send_rolechange_msg(bot = self.bot, discord_id=row[0], notikums='inactive', osu_user=osu_user)
                                continue
                            if ROLES[current_role[0]] != ROLES['inactive']:
                                await change_role(bot = self.bot, discord_id = row[0], current_role_id=ROLES[current_role[0]], new_role_id=ROLES['inactive'])
                                await send_rolechange_msg(bot = self.bot, discord_id=row[0], notikums='inactive', osu_user=osu_user)
                            continue
                    ...
                    if current_role == []:
                        await change_role(bot = self.bot, discord_id=row[0], new_role_id=ROLES[new_role])
                        await send_rolechange_msg(bot = self.bot, discord_id=row[0], notikums='no_previous_role', role=new_role, osu_id=row[1])

                    elif current_role[0] == 'restricted':
                        await change_role(bot = self.bot, discord_id=row[0], current_role_id=ROLES[current_role[0]], new_role_id=ROLES[new_role])
                        await send_rolechange_msg(bot = self.bot, discord_id=row[0], notikums='unrestricted', role=new_role, osu_id=row[1])
                   
                    elif new_role != current_role[0]:
                        if ROLES_VALUE[new_role] < ROLES_VALUE[current_role[0]]:
                            await change_role(bot = self.bot, discord_id=row[0], current_role_id=ROLES[current_role[0]], new_role_id=ROLES[new_role])
                            await send_rolechange_msg(bot = self.bot, discord_id=row[0], notikums='pacelas', role=new_role, osu_id=row[1])
                            continue
                        if ROLES_VALUE[new_role] > ROLES_VALUE[current_role[0]]:
                            await change_role(bot = self.bot, discord_id=row[0], current_role_id=ROLES[current_role[0]], new_role_id=ROLES[new_role])
                            await send_rolechange_msg(bot = self.bot, discord_id=row[0], notikums='nokritas', role=new_role, osu_id=row[1])
                            continue
