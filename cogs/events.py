...
class Events(commands.Cog):
    ...
    async def on_member_join(self, member):
        async with self.bot.pool.acquire() as db:
            result = await get_user(db, member.id)
            message = (f'{member.mention} pievienojās serverim!' if not result 
                      else f'{member.mention} atkal pievienojās serverim!')
            
            if not result:
                await create_user(db, member.id)
    ...
    async def on_member_ban(self, guild, member):
        await self._send_notification(f'**{member.display_name}** ir ticis nobanots no servera!')

    @commands.Cog.listener()
    async def on_member_unban(self, guild, member):
        await self._send_notification(f'**{member.display_name}** ir unbanots no servera!')
